class ProgressBar(object):
    ...
    def set_variables(self, current=0, buffer=1, skip=False):
        self.current = current
        self.buffer = buffer
        self.skip = skip
        self.completed = self.current == self.buffer

    def percentage_completed(self):
        percentage = int(self.current * 100) / self.buffer
    ...
    def right_text(self):
        return "{} {}".format(self.percentage_format(), self.balance_format())
    
    def full_right_text(self):
        return "{}{}".format(self.right_text(), self.skip_format())

    def full_text(self):
        return "{}{}{}".format(self.prefix, self.right_text(), " " * 4)

    def full_progress_bar(self):
        return "\r{} |{}| {}".format(self.prefix, self.progress_bar(), self.full_right_text())
    ...
    def is_valid_show_balance(self):
        return self.buffer != 1 and self.current != 0 if self.show_balance else False
        
    def complete(self):
        self.set_variables(self.buffer, self.buffer)
        self.show_balance = self.is_valid_show_balance()
